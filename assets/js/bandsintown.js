...
function searchBandsInTownVenue(artist) {
  ...
  $.ajax({
    ...
    for (let i = 0; i < response.length; i++) {
      ...
      let buyTicket = $("<a>");
      //parse date to local
      let dateToLOcal = moment.parseZone(response[i].datetime).local().format("LLL"); // "November 2, 2019 12:30 PM"
      ...
      <br><span class="text-muted">At ${response[i].venue.name}</span></h6>`);
